...
| [Circuit migration](CircuitMigration.md) | Moving every circuit development to CSLib's circuit model: redefined measures and families, the converse translation, and consumer ports | On `circuit-migration`: the forward translation of phase 1 (same function, size, gated outputs, and total fan-in) and steps 2.1–2.2 are done; next is the converse translation, which preserves size exactly only for single-output circuits; the rest is planned |
