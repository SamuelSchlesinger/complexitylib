...
algebraic-circuits library: parity bounds in the De Morgan basis and in
$\cc{AC}^0$, monotone CLIQUE, Nechiporuk's formula bound, Karchmer--Wigderson,
a circuit-size hierarchy, conditional complexity, algebraic lower bounds, and
mass production. Its circuit results use CSLib's circuits, each over the
imported library's own signature and cost model; Nechiporuk's bound and
Karchmer--Wigderson are about formulas, and the Waring bound is an identity of
polynomials.
...
constants, negation, conjunction, and disjunction; the parity bound charges
only conjunctions and disjunctions, while the hierarchy counts every gate; its
$\cc{AC}^0$ circuits have unbounded fan-in, with negations free and adding no
depth. Each node names its model and what it counts. Nechiporuk's bound and
Karchmer--Wigderson are stated for the imported library's formula types, not
for circuits, and the Waring bound is an identity of polynomials with no
circuit in its statement. Conditional complexity is defined in this library,
...
\begin{theorem}[Nechiporuk]
  ...
  elements), with $K(n) \le n^c$, and has at least $2^{n - 2}$ accepting
  inputs. Then for all large $n$ every formula over the full binary basis
  computing the length-$n$ member has at least $n^2 / (64(c + 3)\log_2 n)$
  leaves. This is a conditional statement about any such family: no explicit
  rectangle-free family is formalized.
\end{theorem}
\begin{proof}
  \leanok
...
\begin{theorem}[Karchmer--Wigderson]
  ...
  \leanok
  For every $f$ on $n \ge 1$ inputs, the minimum depth and size of a De Morgan
  formula for $f$ equal the minimum depth and number of leaves of a protocol
  for the Karchmer--Wigderson game of $f$. At $n = 0$ the game has no
  protocol, since a protocol answers with an input position.
\end{theorem}
\begin{proof}
  \leanok
...
\begin{theorem}[Laws of conditional complexity]
  ...
  $f$ costs at most computing $h$ given $g$ plus computing $g$ given $f$.
  Over $\mathbb{F}_2$, take any basis whose gates have fan-in at most two and
  in which $x_0 + x_1$ is computed by a single gate. Then the conditional
  ...
  whose sum is the target, minus one. The basis may contain other operations
  as well; the single-gate addition is a hypothesis of the theorem.
\end{theorem}
\begin{proof}
  \leanok
...
\begin{theorem}[Cutwidth bound, conditional]
  ...
  and at least $2^{n-2}$ accepting inputs, as in
  Theorem~\ref{thm:lowerbounds-algebraic-nechiporuk}, needs more than
  $(4 - \varepsilon)n$ gates over the full binary basis, for every
  $\varepsilon > 0$ and all large $n$. The pathwidth bound is an explicit
...
\begin{definition}[The KRW conjecture]
  ...
  of a De Morgan formula computing $f$. For $f$ on $m$ bits and $g$ on $n$
