...
\begin{theorem}[Time classes on CSLib's multi-tape machines]
  ...
  $(2k + 3)(2f + 5)$. CSLib's machines take their alphabet as a parameter,
  and its complexity definitions use machines over the binary alphabet with
  finitely many states: a read-only input tape clamped to the input, two-way
  work tapes that start blank, and emitted output, with the work cells visited
  counted as space. Consequently every
...
\begin{theorem}[CSLib's polynomial time is $\cc{P}$]
  ...
  A CSLib multi-tape machine deciding $L$ within time $t(|x|)$, under any
  space bound, yields one of our machines deciding $L$ within time $3t$.
...
\begin{proof}
  ...
  their tape symbols (blank, $0$, $1$) are already ours. Fold each two-way work tape onto a one-sided tape,
