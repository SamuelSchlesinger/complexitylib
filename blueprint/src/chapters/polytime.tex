...
\begin{theorem}[Regular languages are in $\cc{P}$]
  ...
  $\cc{DTIME}(n + 2)$ and hence in $\cc{P}$. In particular, the language of any
  finite nondeterministic automaton from CSLib's automata library is in $\cc{P}$.
\end{theorem}
\begin{proof}
  \leanok
  A finite-state scanner machine runs the deterministic automaton over the
  input in one pass and writes whether the final state accepts. For a
  nondeterministic automaton, CSLib's subset construction first shows that its
  language is regular. CSLib's closure properties of regular languages therefore
