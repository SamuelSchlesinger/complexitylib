...
\begin{theorem}[Circuits as straight-line programs]
  ...
  \lean{Complexity.Circuit.toStraightLine, Complexity.Circuit.eval_toStraightLine,
    Complexity.Circuit.size_toStraightLine}
  \leanok
  \uses{def:circuit, def:circuits-bases}
  Every basis $B$ is a CSLib signature whose operation symbols are the gates of
  $B$: an operation, a fan-in it allows, and a negation flag for each input,
  interpreted exactly as the gate evaluates. A circuit over $B$ with $G$
  internal gates and $M$ outputs becomes a CSLib circuit over this signature
  that computes the same function and has exactly $G + M$ gates, its size.
\end{theorem}
\begin{proof}
  \leanok
...
\begin{theorem}[CSLib circuits]
  ...
  of $L$ is decided by a CSLib circuit with at most $p(n)$ gates, so
  $\cc{P/poly}$ is exactly CSLib's $\cc{P/poly}$. The size classes interleave:
  $\cc{SIZE}(s)$ lies in CSLib's $\cc{SIZE}(n + 2s + 1)$, and CSLib's
  $\cc{SIZE}(s)$ lies in $\cc{SIZE}(s + 1)$.
\end{theorem}
\begin{proof}
  \leanok
