...
\begin{theorem}[Circuits as straight-line programs]
  ...
  Every output of the translation is an internal gate, and its total fan-in is
  that of the original circuit. The translation goes one way: the converse,
  from CSLib circuits whose outputs are internal gates, is planned.
\end{theorem}
\begin{proof}
  \leanok
...
\begin{theorem}[CSLib circuits]
  ...
  negation, and binary conjunction and disjunction, every gate counted) with
  $N$ inputs, $g$ gates, and $M$ outputs is a fan-in-two AND/OR circuit of
  size $g + M$ computing the same outputs. Conversely, a fan-in-two AND/OR
  circuit with $N$ inputs, $G$ internal gates, and $M$ outputs has a CSLib
  circuit with at most $N + 2G + M$ gates computing the same outputs.
...
\begin{theorem}[Depth in CSLib's circuit model]
  ...
  Let $N, M \ge 1$, as in Theorem~\ref{thm:circuits-cslib}. The translation
  of a CSLib De Morgan circuit has exactly one more level of depth, for its
  output gates. Conversely, a fan-in-two AND/OR circuit with $N$ inputs, $G$
  internal gates, and $M$ outputs has a CSLib circuit with exactly
