...
theorem andOr2_eval_two {W : Nat} (g : Gate Basis.andOr2 W)
  ...
  cases g.op <;> simp_all [AndOrOp.eval, Fin.foldl_succ_last, Fin.foldl_zero] <;>
    congr 1
  ...
  if hw0 : w0.val = 0 then
    if hw1 : w1.val = 0 then
      let result := match g.op with
        | .and => eff0 && eff1
        | .or => eff0 || eff1
      mkConstGateBounded (k := k) (m := m) (G := G) result bound
    else
      let w1' : Fin (k + m + G) := ⟨w1.val - 1, by omega⟩
      have hw1'_bound : w1'.val < k + m + bound := by simp [w1']; omega
      match g.op with
      | .and =>
        if eff0 then mkIdentGateBounded .and w1' n1 bound hw1'_bound
        else mkConstGateBounded (k := k) (m := m) (G := G) false bound
      | .or =>
        if eff0 then mkConstGateBounded (k := k) (m := m) (G := G) true bound
        else mkIdentGateBounded .or w1' n1 bound hw1'_bound
  else
    if hw1 : w1.val = 0 then
      ...
      match g.op with
      | .and =>
        if eff1 then mkIdentGateBounded .and w0' n0 bound hw0'_bound
        else mkConstGateBounded (k := k) (m := m) (G := G) false bound
      | .or =>
        if eff1 then mkConstGateBounded (k := k) (m := m) (G := G) true bound
        else mkIdentGateBounded .or w0' n0 bound hw0'_bound
    else
...
def restrictCircuit {G : Nat} [NeZero m] (b : Bool)
  ...
  acyclic i := (restrictGateBounded (k := k) (m := m) b (c.gates i) (fanIn_andOr2 (c.gates i))
...
theorem restrictGateP_eval_eq {G : Nat} [NeZero m]
    ...
    g.eval wvOld =
    (restrictGateBounded (k := k) (m := m) b g hfanIn bound hbound).val.eval wvNew := by
  ...
  | _ w ih =>
    ...
    · push Not at hwN
      ...
      have hg_eq : (⟨w - 1 - (k + m), by omega⟩ : Fin G) = ⟨w - (k + 1 + m), by omega⟩ := by
        ext; exact hidx
      simp only [hg_eq]
      apply restrictGateP_eval_eq b (c.gates ⟨w-(k+1+m), by omega⟩) (fanIn_andOr2 _)
        (w-1-(k+m)) (by omega)
