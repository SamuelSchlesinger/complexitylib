...
This module holds the **proof machinery** for the RTM → TM compiler: the trivial
empty-output machine, the layout-relative compiler interface
(`LoadedStart` / `CompilesUnder` / `Compiled`), the concrete per-constructor
machines and their compiler parts, and the structural recursion
`compilesUnder_of_inPlace` that assembles them. Correctness is established by the
type checker; the human-auditable contract it discharges lives in
...
- `RoseTreeMachine.emptyOutputTM` — the one-state machine that halts immediately
  with empty output, with its time/space computation lemmas.
...
- `RoseTreeMachine.compiled_var`, `compiled_empty`, `compiled_cons`,
  ...
  **individual compiler parts**, one per `InPlace` constructor. `compiled_empty`
  is proven; each remaining part is stated but still `sorry`: it needs concrete
  `Data`-manipulation subroutines over the tape serialization, tracked as future
  work in `ROADMAP.md` (track N0).
- `RoseTreeMachine.compilesUnder_of_inPlace` — **the internal recursion**: every
  ...
  layout satisfying `CompilesUnder`. The assembly is complete (no `sorry`); it
  dispatches each program constructor to its individual part.

> Note: the `compiled_*` parts use `sorry` (and `compilesUnder_of_inPlace`
> depends on the parts), so `lake build --wfail` and the axiom guard will report
> them until the parts are proved.
-/
